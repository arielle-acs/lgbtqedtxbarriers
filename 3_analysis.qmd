...
#This computes the N and within-group percent by gender group needed for the tests below (cis % is out of the cis n, trans % is out of the trans n)
...
#Step 4 = calculate odd's ratio estimate and 95% CI
...
##Step 4
fisher.test(code1_table)$estimate
fisher.test(code1_table)$conf.int
...
##Step 4
fisher.test(code2_table)$estimate
fisher.test(code2_table)$conf.int
...
##Step 4
fisher.test(code3_table)$estimate
fisher.test(code3_table)$conf.int
...
##Step 4
fisher.test(code4_table)$estimate
fisher.test(code4_table)$conf.int
...
##Step 4
fisher.test(code5_table)$estimate
fisher.test(code5_table)$conf.int
...
fisher.test(code6_table)$estimate
fisher.test(code6_table)$conf.int
...
##Step 4
fisher.test(code7_table)$estimate
fisher.test(code7_table)$conf.int
...
##Step 4
fisher.test(code8_table)$estimate
fisher.test(code8_table)$conf.int
...
##Step 4
fisher.test(code9_table)$estimate
fisher.test(code9_table)$conf.int
...
##Step 4
fisher.test(code10_table)$estimate
fisher.test(code10_table)$conf.int
...
##Step 4
fisher.test(code11_table)$estimate
fisher.test(code11_table)$conf.int
```
...
```{r n and percent for number of codes endorsed}
demographics_and_codes %>%
  mutate(row_sum = rowSums(across(Code_1:Code_11))) %>%
  summarise(
    sum_1 = sum(row_sum == 1),
    percent_1 = sum(row_sum == 1) / n() * 100,
    sum_2 = sum(row_sum == 2),
    percent_2 = sum(row_sum == 2) / n() * 100,
    sum_3 = sum(row_sum == 3),
    percent_3 = sum(row_sum == 3) / n() * 100
  )
```
